...
'''
with open("Package.txt", "wb") as outfile:
	for f in read_files:
...
with open("Package.json", "wb") as outfile:
	for f in read_files:
		lst = list()
		source = None
					...
					if (groups[i] == 'FROM' or groups[i] == 'JOIN'): 
						try:
							...
							if table.find('dbo.') >= 0:
								table = table[table.find('dbo.')+4:len(table)]
							if table.find('#') >= 0: continue
							if not table in lst and not ( table.find('SELECT') >= 0  or table.find('split') >= 0 or table.find('Temp') >= 0):
								lst.append(table)
						except:
							T = 1
							break
		#write into a json file. The reason I don't use txt file is because there is a list which can not write into a txt file
...
fl = open('Package.JSON').readlines()
...
for f in fl:
	...
	col2 = f[1]
	col3 = str(f[2])[1:-1]
	sql_command = ("INSERT INTO Package (PackageName, SourceDB, TableList) VALUES (?,?,?)")	
...
sql_command =   """
SELECT PackageName, SourceDB, TableList FROM Package
"""
try:
	db_cursor.execute(sql_command)
except pyodbc.ProgrammingError:
...
while row:
	##We only select two columns so your index should not great than 1
